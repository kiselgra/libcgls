...
(define (make-cpu-mmsm)
  (let* ((head-tex (find-texture "shadow_head_buffer"))
	 ...
	 (mipmap
	  (lambda (to)
	    (format #t "mipmapping...~%")
	    (vector-set! heads 1 (make-bytevector (bytevector-length (vector-ref heads 0))))
	    (vector-set! tails 1 (make-bytevector (bytevector-length (vector-ref tails 0))))
	    (vector-set! depths 1 (make-bytevector (bytevector-length (vector-ref depths 0))))
	    (let ((from-head (vector-ref heads 0))
		  (from-tail (vector-ref tails 0))
		  (from-depth (vector-ref depths 0))
		  (to-head (vector-ref heads 1))
		  (to-tail (vector-ref tails 1))
		  (to-depth (vector-ref depths 1))
		  (counter 0))
	      (receive (w h) (mmsm 'size)
		(for-each-pixel
		 (lambda (w h x y)
		   (let ((from-x (* x 2)) (from-y (* y 2))
		     ...
		     (let merge ((curr-id-00 (int from-head (+ (* from-y from-w) from-x)))
				 (curr-id-01 (int from-head (+ (* from-y from-w) from-x 1)))
				 (curr-id-10 (int from-head (+ (* (1+ from-y) from-w) from-x)))
				 (curr-id-11 (int from-head (+ (* (1+ from-y) from-w) from-x 1))))
		       (let* ((d-00 (if (>= curr-id-00 0) (float from-depth (index->pixel-offset curr-id-00 from-w from-h)) 9999))
			      (d-01 (if (>= curr-id-01 0) (float from-depth (index->pixel-offset curr-id-01 from-w from-h)) 9999))
			      (d-10 (if (>= curr-id-10 0) (float from-depth (index->pixel-offset curr-id-10 from-w from-h)) 9999))
			      (d-11 (if (>= curr-id-11 0) (float from-depth (index->pixel-offset curr-id-11 from-w from-h)) 9999))
			      ;(min-d (min d-00 d-01 d-10 d-11))
			      (min-d (max d-00 d-01 d-10 d-11))
			      (reg (lambda (depth-val)
				     (let ((old-pos counter)
					   (new-head (1+ counter)))
				       (set! counter (1+ counter))
				       (float! to-depth (index->pixel-offset new-head w h) depth-val)
				       (int! to-tail (index->pixel-offset new-head w h) old-pos)
				       (int! to-head (+ (* y w) x) new-head)))))
			 (cond ((= min-d 9999) 'done)
			       ((= min-d d-00) (reg min-d) (merge (int from-tail (index->pixel-offset curr-id-00 from-w from-h)) curr-id-01 curr-id-10 curr-id-11))
			       ((= min-d d-01) (reg min-d) (merge curr-id-00 (int from-tail (index->pixel-offset curr-id-01 from-w from-h)) curr-id-10 curr-id-11))
			       ((= min-d d-10) (reg min-d) (merge curr-id-00 curr-id-01 (int from-tail (index->pixel-offset curr-id-10 from-w from-h)) curr-id-11))
			       ((= min-d d-11) (reg min-d) (merge curr-id-00 curr-id-01 curr-id-10 (int from-tail (index->pixel-offset curr-id-11 from-w from-h))))))
		       )))
		 (/ w 2) (/ h 2)))
	      )))
	 )
