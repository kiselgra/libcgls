  ...
  (let* ((head-tex (make-vector 10)) ;(find-texture "shadow_head_buffer-0"))
	 ...
	 (check-all-sorted (lambda (level)
				 ...
				 (for-each-pixel (lambda (w h x y)
						   (let traverse-tail ((d 2) (link (int (vector-ref heads level) (+ (* y w) x))) (i 0))
						     (if (and (= x 0) (= y 0))
					 ...
					 (if (< id 0)
					     ...
					     (collect (int from-tail (index->pixel-offset id from-w from-h))
						      (cons (list (float from-depth (index->pixel-offset id from-w from-h))
								  id)
							    accum))))))
			    ; we make the list ascending to easily produce a descending list afterwards.
			    (descending (sort (append (collect 0 0) (collect 0 1) (collect 1 0) (collect 1 1)) 
					      (lambda (a b) (> (car a) (car b)))))
			    (epsilon (mmsm 'epsilon))
			    (ascending (fold (lambda (new so-far)
					       (if (> (- (caar so-far) (car new)) epsilon)
						   (cons new so-far)
						   so-far))
					     (list (car descending))
					     (cdr descending))))
		       (let register ((depth (caar ascending)) (id (cadar ascending)) (prev -1) (rest ascending))
