...
(define (make-mm-shadow-buffers name w h storage-factor)
  ...
  (let ((frag-head-name (string-append name "_" "head_buffer"))
    ...
    (let ((frag-heads (make-vector 10))
      ...
      (let init ((i 0))
	(when (< i 10)
	  (vector-set! frag-heads i (make-texture-without-file (format #f "~a-~a" frag-head-name i) gl#texture-2d w h gl#red gl#r32f gl#float))
...
(define (make-cpu-mmsm)
  (let* ((head-tex (find-texture "shadow_head_buffer-0"))
	 (tail-tex (find-texture "shadow_tail_buffer-0"))
	 (depth-tex (find-texture "shadow_frag_depths-0"))
	 ...
	 (download (lambda ()
		     ...
		     (vector-set! alphas 0 (make-bytevector (bytevector-length (vector-ref depths 0))))
		     (let zero ((i 0) (l (/ (bytevector-length (vector-ref alphas 0)) 4)))
		       (if (< i l)
			   (float! (vector-ref alphas 0) i 1)
			   (zero (1+ i) l)))))
  ...
  (let ((texquad (find-drawelement "texquad"))
    ...
    (receive (w h) (mmsm 'size)
      (make-pass "mip map one step starting from a given mmsm level." '()
		 ...
		 (begin
		   (mmsm 'level! 0)
		   (let ((w (/ w (expt 2 (mmsm 'level))))
		         (h (/ h (expt 2 (mmsm 'level)))))
		     ;(disable-color-output
		     (gl:disable gl#depth-test)
		      (disable-depth-output
		       (with-viewport (0 0 w h)
